...
function CustomReports() {
    ...
    const fetchFreelancers = async () => {
        try {
            const response = await fetch('https://api.conexaocode.com/api/freelancers2');
            const json = await response.json();
    ...
    const generateReport = async () => {
        ...
        try {
            ...
            const [reportResponse, statsResponse, statusResponse] = await Promise.all([
                fetch(`https://api.conexaocode.com/api/reports/data?${params}`),
                fetch(`https://api.conexaocode.com/api/reports/stats?${params}`),
                fetch(`https://api.conexaocode.com/api/reports/status?${params}`),
            ]);
    ...
    const exportReport = async (format: 'excel' | 'pdf') => {
        try {
            ...
            const response = await fetch(`https://api.conexaocode.com/api/reports/export?${params}`);
